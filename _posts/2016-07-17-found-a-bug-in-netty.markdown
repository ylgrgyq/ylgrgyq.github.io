...
从 dump 中我们还发现，维护 OpenSslClientContext 的业务对象是没有泄露的，有被正常 GC 。即我们的业务代码是确认有正确清理指向 OpenSslClientContext 对象的引用的。那这个 OpenSslClientContext 是怎么被 GC Root 引用到的呢？
